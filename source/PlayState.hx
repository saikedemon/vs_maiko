...
class PlayState extends MusicBeatState
{
	...
	var spungebub:FlxSprite = new FlxSprite(-100, -100).loadGraphic(Paths.image('city/spungebub'));
	var spungebubTween:FlxTween;
	var cameos:BGSprite;
	...
	private var stupidThingBecauseCodeDoesntWork:Bool = false;
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'ace' | 'maiko' | 'bliss' | 'magician':
				curStage = 'redcity';

				defaultCamZoom = 0.8;

				var city:BGSprite = new BGSprite('city/bg', -500, -250, 0.9, 0.9);
				city.setGraphicSize(Std.int(city.width * 1));
				city.updateHitbox();
				add(city);

				cameos = new BGSprite('city/boppers', -330, 570, 1, 1, ['cameos']);
				cameos.setGraphicSize(Std.int(cameos.width * 0.9));
				cameos.updateHitbox();
	...
	function endSong():Void
	{
		...
		if (isStoryMode)
		{
			...
			if (storyPlaylist.length <= 0)
			{
				...
				if (SONG.song.toLowerCase() == 'bliss')
				{
					MusicBeatState.switchState(new VideoState("assets/videos/micdrop.webm", new StoryMenuState()));
				}
				else
				{
